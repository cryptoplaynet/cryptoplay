...
const char * const BITCOIN_CONF_FILENAME = "cryptoplay.conf";
const char * const BITCOIN_PID_FILENAME = "cryptoplayd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cryptoplay" is a composite category enabling all CryptoPlay-related debug output
            if(ptrCategory->count(string("cryptoplay"))) {
...
#else
    const char* pszModule = "cryptoplay";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CryptoPlayCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CryptoPlayCore
    // Mac: ~/Library/Application Support/CryptoPlayCore
    // Unix: ~/.cryptoplaycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CryptoPlayCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CryptoPlayCore";
#else
    // Unix
    return pathRet / ".cryptoplaycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cryptoplay.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cryptoplay.conf
