...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CryptoPlay address (e.g. %1)").arg("XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg"));
#endif
...
{
    // return if URI is not valid or is no cryptoplay: URI
    if(!uri.isValid() || uri.scheme() != QString("cryptoplay"))
        return false;
...
{
    // Convert cryptoplay:// to cryptoplay:
    //
    //    Cannot handle this later, because cryptoplay:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("cryptoplay://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "cryptoplay:");
    }
...
{
    QString ret = QString("cryptoplay:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open cryptoplay.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CryptoPlay.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CryptoPlay (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("CryptoPlay (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for CryptoPlay*.lnk
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "cryptoplay.desktop";
    return GetAutostartDir() / strprintf("cryptoplay-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a cryptoplay.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=CryptoPlay\n";
        else
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the CryptoPlay Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add CryptoPlay Core app to startup item list
